...
class CustomModel(nn.Module):
    ...
    Architecture (diagram-aligned):
        ...
        Conv 1×1, 168 → BN, ReLU → MaxPool2×2          (inter-stage bridge)
        AMSFE Stage-2 → output 168ch → MaxPool2×2
        Conv Block  (Conv3×3,224 → BN,ReLU → Conv3×3,224 → BN,ReLU → MaxPool2×2)
        Global Average Pooling → 224-d vector
    ...
    def __init__(self, num_classes=9):
        ...
        # ── Inter-stage bridge Conv 1×1 ──────────────────────────────────────
        self.bridge = nn.Sequential(
            nn.Conv2d(168, 168, kernel_size=1, bias=False),
            nn.BatchNorm2d(168),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2)
        )
        ...
        self.conv_block = nn.Sequential(
            nn.Conv2d(168, 224, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(224),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        x = self.conv_block(x)         # (B, 224, H/32, W/32)

        x = self.global_pool(x)        # (B, 224, 1, 1)
        x = torch.flatten(x, 1)        # (B, 224)
